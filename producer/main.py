...
def main():
    ...
    print(f"Replaying {DATA_PATH} → {TOPIC} at {TX_PER_SECOND} tx/s")
    ...
    with open(DATA_PATH) as f:
        for row in csv.DictReader(f):
            ...
            if count % 10_000 == 0:
                print(f"[producer] sent {count} transactions")

            time.sleep(delay)

    producer.flush()
    print(f"[producer] replay complete — {count} transactions sent")
